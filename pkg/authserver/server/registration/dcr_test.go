...
func TestValidateScopes(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// Narrowed supported-scope sets are honoured rather than rejected:
			// the server's advertised scopes become the DCR default grant set.
			name:            "empty input uses allowed set even when defaults are not a subset",
			requestedScopes: nil,
			allowedScopes:   []string{"custom_scope"},
			expectedScopes:  []string{"custom_scope"},
		},
	}
