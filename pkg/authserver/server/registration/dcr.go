...
//   - Empty/nil input falls back to the server defaults (see
//     defaultScopesForServer): RFC 7591 leaves scope selection to the server
//     when the client omits the field. ToolHive does not expose a separate
//     "default scopes" setting, so when the server advertises an explicit
//     supported-scope set we treat that as the default grant set for DCR.
//     This keeps dynamic registration aligned with the protected-resource
//     metadata and avoids hard failures when deployments intentionally narrow
//     the supported scopes. With no supported-scope set configured, the
//     built-in DefaultScopes are used.
...
// defaultScopesForServer derives the scope set granted when a DCR request omits
// the scope field (RFC 7591 §2 leaves this to the server).
