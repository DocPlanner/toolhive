...
func (d *CIMDStorageDecorator) fetch(ctx context.Context, id string) (fosite.Client, error) {
	...
	//     the server's default grant set, matching DCR: DefaultScopes when no
	//     supported set is configured, otherwise the configured ScopesSupported
	//     set itself (deployments that narrow ScopesSupported get that narrowed
	//     set rather than a hard failure).
	...
	if len(d.scopesSupported) > 0 {
		if doc.Scope != "" {
			...
			// Omitted scope: match DCR — the server's default grant set derived
			// from the configured ScopesSupported.
