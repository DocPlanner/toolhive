...
func TestFetch_ScopeResolution(t *testing.T) {
	...
	tests := []struct {
		...
		{
			// A narrowed ScopesSupported set becomes the default grant set
			// (matching registration.ValidateScopes) rather than a hard failure.
			name:            "omitted scope with restrictive ScopesSupported uses that set",
			docScope:        "",
			scopesSupported: []string{"openid"},
			wantScopes:      []string{"openid"},
		},
