...
func marshalUpstreamTokensWithTTL(tokens *UpstreamTokens) ([]byte, time.Duration, error) {
	...
	// Zero ExpiresAt means the token never expires; return ttl=0 so the caller
...
//
//nolint:gocyclo // WATCH/MULTI retry loop plus index-TTL maintenance; native-command path for Dragonfly
func (s *RedisStorage) StoreUpstreamTokens(ctx context.Context, sessionID, providerName string, tokens *UpstreamTokens) error {
	...
	for attempt := 0; attempt < maxStoreUpstreamTokensRetries; attempt++ {
		err = s.client.Watch(ctx, func(tx *redis.Tx) error {
			...
			// Read the index set's existence and TTL BEFORE the SADD below.
			// After a SADD, a "no TTL" state is ambiguous: it could be a set we
			// just created (we own the TTL decision) or an existing set that a
			// previous non-expiring member deliberately PERSIST'd (must stay
			// persistent). Reading pre-SADD disambiguates the two.
			idxExisted, existsErr := tx.Exists(ctx, idxKey).Result()
			if existsErr != nil && !errors.Is(existsErr, redis.Nil) {
				return fmt.Errorf("failed to read upstream token index: %w", existsErr)
			}
			...
			_, pipeErr := tx.TxPipelined(ctx, func(pipe redis.Pipeliner) error {
				...
				// Invariant: the index set must outlive every per-provider key
				// it points to.
				//   * If this member never expires (ttl == 0), the index set
				//     must be persistent too, otherwise the index evicts and the
				//     per-provider key leaks with no way to find it.
				//   * If all members expire, the index TTL must be at least the
				//     longest member TTL.
				//
				// Known trade-off: an in-place rewrite of the same
				// (sessionID, providerName) slot from non-expiring to expiring
				// leaves the index PERSIST'd even though the rewritten member was
				// the sole persistence anchor. Tracking per-member TTL state in
				// the index would be needed to detect this; DeleteUpstreamTokens
				// and session GC clean the index up anyway.
				switch {
				case ttl == 0:
					// Non-expiring member: make the index persistent (no-op if already so).
					pipe.Persist(ctx, idxKey)
				case idxExisted == 0:
					// Fresh set created by our SADD — we own the TTL decision.
					pipe.PExpire(ctx, idxKey, ttl)
				case currentTTL == -1:
					// Existing set previously PERSIST'd by a non-expiring member.
					// Leave it alone — applying a TTL here would reintroduce the leak.
				case currentTTL < ttl:
					// Existing TTL is shorter than this member's — extend it.
					pipe.PExpire(ctx, idxKey, ttl)
				}
