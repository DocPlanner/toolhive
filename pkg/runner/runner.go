...
// defaultProxySessionKeyPrefix is used when the RunConfig does not carry an
// explicit Redis key prefix for proxy sessions.
const defaultProxySessionKeyPrefix = "thv:proxy:session:"

// newTransportSessionStorage builds the Redis-backed proxy session store.
// ttl is the already-resolved effective session TTL, so the proxy and the
// storage layer cannot drift apart. Credentials come from the environment:
// the ACL username (fork behavior, needed for Redis 6+ ACL users provisioned
// by the operator) plus the password injected as a SecretKeyRef.
...
func (r *Runner) Run(ctx context.Context) error {
	...
	// The env-var override (TOOLHIVE_PROXY_SESSION_TTL, DocPlanner fork behavior)
	// supplies the baseline; an explicit RunConfig SessionTTL still wins.
