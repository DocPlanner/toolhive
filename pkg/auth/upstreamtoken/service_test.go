...
func TestInProcessService_GetAllUpstreamCredentials(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupRefresher: func(_ *storagemocks.MockUpstreamTokenRefresher) {},
			wantResult: map[string]UpstreamCredential{
				"github":    {AccessToken: "github-access-token", IDToken: "github-id-token", Status: StatusValid},
				"atlassian": {AccessToken: "atlassian-access-token", IDToken: "atlassian-id-token", Status: StatusValid},
			},
		},
		{
			...
			wantResult: map[string]UpstreamCredential{
				"atlassian": {AccessToken: "atlassian-access-token", IDToken: "atlassian-id-token", Status: StatusValid},
				"github":    {AccessToken: "new-github-token", IDToken: "github-id-token", Status: StatusValid},
			},
		},
		{
			...
			wantResult: map[string]UpstreamCredential{
				"github": {AccessToken: "new-github-token", IDToken: "rotated-github-id-token", Status: StatusValid},
			},
		},
		{
			name:      "expired refresh fails preserves provider status and reports it in failed slice",
			sessionID: "session-3",
			setupStorage: func(s *storagemocks.MockUpstreamTokenStorage) {
		...
		},
		{
			...
			wantResult: map[string]UpstreamCredential{
				"github":    {Status: StatusRefreshFailed},
				"atlassian": {Status: StatusRefreshFailed},
			},
		...
		{
			name:      "expired with no refresh token preserves provider status and reports it in failed slice",
			sessionID: "session-7",
			setupStorage: func(s *storagemocks.MockUpstreamTokenStorage) {
		...
		},
		{
			...
			setupRefresher: func(_ *storagemocks.MockUpstreamTokenRefresher) {},
			wantResult: map[string]UpstreamCredential{
				"github": {AccessToken: "no-expiry-token", IDToken: "github-id-token", Status: StatusValid},
			},
		},
		{
			...
			setupRefresher: func(_ *storagemocks.MockUpstreamTokenRefresher) {},
			wantResult: map[string]UpstreamCredential{
				"github": {AccessToken: "no-id-token", IDToken: "", Status: StatusValid},
			},
		},
		{
			...
			setupRefresher: func(_ *storagemocks.MockUpstreamTokenRefresher) {},
			wantResult: map[string]UpstreamCredential{
				"github":    {AccessToken: "gh-access", IDToken: "", Status: StatusValid},
				"atlassian": {AccessToken: "atl-access", IDToken: "", Status: StatusValid},
			},
		},
	}
...
// when the refresher is nil, expired tokens in the bulk path are preserved as
