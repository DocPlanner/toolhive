...
func (m *Monitor) performHealthCheck(ctx context.Context, backend *vmcp.Backend) {
	...
	target := &vmcp.BackendTarget{
		...
		// Propagate the reconciled workload status so the checker can use the
		// managed-workload fast path for Kubernetes MCPServer backends instead
		// of opening an MCP session on every periodic check.
