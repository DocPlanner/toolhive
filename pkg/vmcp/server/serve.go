...
func Serve(ctx context.Context, v core.VMCP, cfg *ServerConfig) (*Server, error) {
	...
	mcpServer := server.NewMCPServer(
		serveCfg.Name,
		serveCfg.Version,
		// Session-scoped capability refresh (backend health transitions and
		// backend tools/list_changed notifications) replaces the per-session
		// tool/resource set live, so list_changed capabilities must be advertised.
	...
	// The core owns the backend health monitor; hand its status provider to the
	// session manager so session creation/refresh only binds backends that are
	// currently eligible for exposure.
	var healthStatusProvider health.StatusProvider
	if reporter := v.BackendHealth(); reporter != nil {
		healthStatusProvider = reporter
	}

	vmcpSessMgr, optimizerCleanup, err := sessionmanager.New(
		sessionDataStorage, cfg.SessionManagerConfig, cfg.BackendRegistry, healthStatusProvider)
	if err != nil {
		return nil, err
	}
	...
	// Owner-aware session forwarding (multi-replica): advertise this pod so
	// siblings can proxy live-session traffic back to the owner.
	sessionOwnerURL, err := normalizeSessionOwnerURL(cfg.SessionOwnerAdvertiseURL)
	if err != nil {
		return nil, fmt.Errorf("%w: invalid session owner advertise URL: %w", vmcp.ErrInvalidConfig, err)
	}
	...
	// different replica than the one that originally registered them, and wait
	// briefly for an in-flight post-initialize registration instead of racing a
	// still-empty SDK session.
	hooks.AddOnRequestInitialization(func(hookCtx context.Context, id any, message any) error {
		return srv.handleSessionRequestInitialization(hookCtx, id, message)
	})
...
func buildServeConfig(cfg *ServerConfig) *Config {
	return &Config{
		...
		SessionOwnerAdvertiseURL: cfg.SessionOwnerAdvertiseURL,
