...
func TestSessionManager_Generate(t *testing.T) {
	...
	t.Run("returns empty string when storage always fails", func(t *testing.T) {
		...
		sess := newMockSession(t, ctrl, "placeholder", nil)
		factory := newMockFactory(t, ctrl, sess)
		sm, cleanup, err := New(alwaysFailDataStorage{}, &FactoryConfig{Base: factory, CacheCapacity: 1000}, newFakeRegistry(), nil)
		require.NoError(t, err)
		t.Cleanup(func() { _ = cleanup(context.Background()) })
...
func TestSessionManager_ReplaceSession(t *testing.T) {
	...
	t.Run("does not recreate a terminated session", func(t *testing.T) {
		...
		// Seed the identity-binding key so Terminate classifies this as a
		// phase-2 (full MultiSession) record and deletes it from storage.
		original := sessionmocks.NewMockMultiSession(ctrl)
		original.EXPECT().ID().Return(sessionID).AnyTimes()
		original.EXPECT().GetMetadata().Return(map[string]string{
			sessiontypes.MetadataKeyIdentityBinding: binding.UnauthenticatedSentinel,
		}).AnyTimes()
		original.EXPECT().SetMetadata(gomock.Any(), gomock.Any()).AnyTimes()
		// Backend connections are released lazily when the cache entry is evicted,
		// not synchronously by Terminate.
		original.EXPECT().Close().Return(nil).AnyTimes()
		replacement := newMockSession(t, ctrl, sessionID, nil)
		require.NoError(t, sm.StoreSession(original))
...
func TestSessionManager_Terminate(t *testing.T) {
	...
	t.Run("placeholder termination falls back to delete when upsert fails", func(t *testing.T) {
		...
		sm, cleanup, err := New(failingStorage, &FactoryConfig{Base: factory, CacheCapacity: 1000}, registry, nil)
		require.NoError(t, err)
		t.Cleanup(func() { _ = cleanup(context.Background()) })
...
func TestSessionManager_GetMultiSession(t *testing.T) {
	...
	t.Run("restore path: concurrent delete between RestoreSession and Update returns ErrSessionNotFound", func(t *testing.T) {
		...
		sm, cleanup, err := New(racyStorage, &FactoryConfig{Base: factory, CacheCapacity: 1000}, newFakeRegistry(), nil)
		require.NoError(t, err)
		t.Cleanup(func() { _ = cleanup(context.Background()) })
	...
	t.Run("restore path: transient Update error is non-fatal, session is still returned", func(t *testing.T) {
		...
		sm, cleanup, err := New(faultyStorage, &FactoryConfig{Base: factory, CacheCapacity: 1000}, newFakeRegistry(), nil)
		require.NoError(t, err)
		t.Cleanup(func() { _ = cleanup(context.Background()) })
