...
func populatedLegacyConfig() *Config {
	...
	return &Config{
		...
		SessionOwnerAdvertiseURL: "http://127.0.0.1:4483/mcp",
...
func TestDeriveServerConfigProjectsTransportFields(t *testing.T) {
	...
	assert.Equal(t, 3*time.Minute, got.WriteTimeout)
	assert.Equal(t, "http://127.0.0.1:4483/mcp", got.SessionOwnerAdvertiseURL)
