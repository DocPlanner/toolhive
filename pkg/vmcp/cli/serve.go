...
const (
	// envVMCPSessionOwnerURL optionally overrides the pod-local URL advertised to
	// sibling replicas for live-session forwarding.
	envVMCPSessionOwnerURL = "THV_VMCP_SESSION_OWNER_URL"
	// envPodIP is the downward-API pod IP used to build the default advertise URL.
	envPodIP = "POD_IP"
	// backendResponseMargin is added on top of the largest configured backend
	// request timeout when deriving the server write deadline, leaving time to
	// write the MCP response after the slowest backend call completes.
...
func Serve(ctx context.Context, cfg ServeConfig) error {
	...
	// Backend init and per-request timeouts are wired from the operational
	// config so a slow backend cannot stall session creation or requests.
	var sessionOpts []vmcpsession.MultiSessionFactoryOption
	if initTimeout := backendInitTimeoutFromConfig(vmcpCfg); initTimeout > 0 {
		sessionOpts = append(sessionOpts, vmcpsession.WithBackendInitTimeout(initTimeout))
	}
	backendRequestTimeout, perWorkloadRequestTimeouts := backendRequestTimeoutsFromConfig(vmcpCfg)
	if backendRequestTimeout > 0 || len(perWorkloadRequestTimeouts) > 0 {
		sessionOpts = append(sessionOpts, vmcpsession.WithBackendRequestTimeouts(
			backendRequestTimeout,
			perWorkloadRequestTimeouts,
		))
	}
	sessionFactory := vmcpsession.NewSessionFactory(outgoingRegistry, sessionOpts...)
	...
	// The --session-ttl flag takes precedence; otherwise fall back to the
	// config-supplied (CRD) sessionTTL so operators can relax session sweeps.
...
// backendInitTimeoutFromConfig derives the per-backend session-initialization
// timeout from operational settings. The health check timeout is preferred;
// the default request timeout is the fallback. Returns 0 when unconfigured.
...
// backendRequestTimeoutsFromConfig extracts the default and per-workload
// backend request timeouts from operational settings.
...
// serverWriteTimeoutFromConfig derives the server-level write deadline from the
// largest configured backend request timeout plus a response-write margin.
// Returns 0 when no backend timeouts are configured (server default applies).
...
// resolveSessionOwnerAdvertiseURL resolves the pod-local URL sibling replicas
// use to forward live-session traffic to the owning pod. An explicit URL wins;
// otherwise the URL is built from the downward-API pod IP and listen port.
